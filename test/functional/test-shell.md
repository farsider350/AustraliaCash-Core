...
The `TestShell` submodule extends the `AustraliaCashTestFramework` functionality to
external interactive environments for prototyping and educational purposes. Just
like `AustraliaCashTestFramework`, the `TestShell` allows the user to:
...
to extend the object lifetime of the underlying `AustraliaCashTestFramework` between
...
## 2. Importing `TestShell` from the AustraliaCash Core repository

We can import the `TestShell` by adding the path of the AustraliaCash Core
...
The `TestShell` inherits all `AustraliaCashTestFramework` members and methods, such
as:
...
`AustraliaCashTestFramework` object. The full set of argument keywords which can be
...
Unlike the `AustraliaCashTestFramework` class, the `TestShell` keeps the underlying
AustraliaCashd subprocesses (nodes) and logging utilities running until the user
explicitly shuts down the `TestShell` object.

During the time between the `setup` and `shutdown` calls, all `bitcoind` node
processes and `AustraliaCashTestFramework` convenience methods can be accessed
interactively.
...
underlying `AustraliaCashTestFramework`:
...
`AustraliaCashTestFramework` class. The user can override these in
