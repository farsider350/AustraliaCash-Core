...
// which are part of the AusCash repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="auscash-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /auscash-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "auscashpay-auscash/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from auscashpay-auscash/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'auscashpay-auscash/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from auscashpay-auscash/develop"
            }
          }

          if (hasCache) {
            sh "cd /auscash-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /auscash-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /auscash-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /auscash-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /auscash-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /auscash-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /auscash-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
