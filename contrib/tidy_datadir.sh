#!/bin/bash
# Copyright (c) 2013 The AustraliaCash Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Australiacash database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Australiacash datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Australiacash datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Australiacash 0.7 datadir."
    ;;
  3)
    echo "Detected Australiacash pre-0.8 datadir."
    ;;
  4)
    echo "Detected Australiacash 0.8 datadir."
    ;;
esac
