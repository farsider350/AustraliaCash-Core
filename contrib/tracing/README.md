...
Tracing (USDT) support for AustraliaCash Core on Linux using. For more information on
USDT support in AustraliaCash Core see the [USDT documentation].
...
[AustraliaCash Core USDT documentation] on how to list available tracepoints in your
`bitcoind` binary.

[AustraliaCash Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
AustraliaCash P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
In a different terminal, starting AustraliaCash Core in SigNet mode and with
re-indexing enabled.
