...
# auscash_hash
RUN git clone https://github.com/auscashpay/auscash_hash
RUN cd auscash_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} auscash
RUN useradd -u ${USER_ID} -g auscash -s /bin/bash -m -d /auscash auscash
...
RUN mkdir /auscash-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /auscash-src && \
