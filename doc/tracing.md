# User-space, Statically Defined Tracing (USDT) for AustraliaCash Core

AustraliaCash Core includes statically defined tracepoints to allow for more
...
**Note**: AustraliaCash Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to AustraliaCash Core
...
To list probes in AustraliaCash Core, use `info probes` in `gdb`:
...
The `readelf` tool can be used to display the USDT tracepoints in AustraliaCash Core.
