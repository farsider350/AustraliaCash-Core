# CJDNS support in AustraliaCash Core

It is possible to run AustraliaCash Core over CJDNS, an encrypted IPv6 network that
...
done outside of AustraliaCash Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the AustraliaCash network and individual nodes.
...
before it will work with your AustraliaCash Core node. This is described in steps
...
## Run AustraliaCash Core with CJDNS

Once you are connected to the CJDNS network, the following AustraliaCash Core
...
When enabled, this option tells AustraliaCash Core that it is running in an
...
IPv6 local network. This helps AustraliaCash Core perform better address management:
...
CJDNS support was added to AustraliaCash Core in version 23.0 and there may be fewer
...
## CJDNS-related information in AustraliaCash Core

There are several ways to see your CJDNS address in AustraliaCash Core:
