AustraliaCash Core version 0.18.0 is now available from:
...
`/Applications/AustraliaCash-Qt` (on Mac) or `bitcoind`/`bitcoin-qt` (on
Linux).
...
recommended to use AustraliaCash Core on unsupported systems.
...
10.10. Additionally, AustraliaCash Core does not yet change appearance when
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoincore.org/en/contact/) the AustraliaCash Core
...
`~bitcoin/.bitcoin`. This change makes AustraliaCash Core more consistent with
...
- A new document introduces AustraliaCash Core's BIP174 [Partially-Signed
  AustraliaCash Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent AustraliaCash-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  AustraliaCash Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when AustraliaCash Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of AustraliaCash Core, subsequently importing
...
- A new `bitcoin-wallet` tool is now distributed alongside AustraliaCash
...
This section describes planned changes to AustraliaCash Core that may affect
other AustraliaCash software and services.

- Since version 0.16.0, AustraliaCash Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with AustraliaCash Core 0.20 (expected about
  a year after 0.18), AustraliaCash Core will default to native segwit
  ...
  to bech32 addresses, and if the AustraliaCash Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in AustraliaCash Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any AustraliaCash Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, AustraliaCash Core
...
- A sub-project of AustraliaCash Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with AustraliaCash Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to AustraliaCash Core's own implementation, although entropy
  gathered by AustraliaCash Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves AustraliaCash Core a
...
- On macOS, AustraliaCash Core now opts out of application CPU throttling
...
- #11625 Add AustraliaCashApplication & RPCConsole tests (ryanofsky)
...
- #11526 Visual Studio build configuration for AustraliaCash Core (sipsorcery)
...
- #12775 Integration of property based testing into AustraliaCash Core (Christewart)
