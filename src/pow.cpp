...
unsigned int DigiShieldV4(const CBlockIndex* pindexLast, const Consensus::Params& params)
{
	...
	nActualTimespan = params.nAveragingTargetTimespanV4 + (nActualTimespan - params.nAveragingTargetTimespanV4);
...
unsigned int GetNextWorkRequiredLegacy(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    // Australiacash: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int CalculateNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
    ...
    bnOld = bnNew;
    // Australiacash: intermediate uint256 can overflow by 1 bit
