...
const char * const BITCOIN_PID_FILENAME = "auscashd.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AusCashCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AusCashCore
    // Mac: ~/Library/Application Support/AusCashCore
    // Unix: ~/.auscashcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AusCashCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AusCashCore";
#else
    // Unix
    return pathRet / ".auscashcore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty auscash.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override auscash.conf
