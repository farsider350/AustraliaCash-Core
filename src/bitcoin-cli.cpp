...
static void SetupCliArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-generate",
                   strprintf("Generate blocks, equivalent to RPC getnewaddress followed by RPC generatetoaddress. Optional positional integer "
                             "arguments are number of blocks to generate (default: %s) and maximum timeout before returning (default: %s), equivalent to "
                             "RPC generatetoaddress nblocks and maxtimeout arguments. Example: bitcoin-cli -generate 4 30",
                             DEFAULT_NBLOCKS, DEFAULT_MAX_TIMEOUT),
